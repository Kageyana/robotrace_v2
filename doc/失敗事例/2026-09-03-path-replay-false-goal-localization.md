...
- 経路制御バージョン6で、PATH REPLAY中の右マーカー回数を停止条件から分離し、経路後半で座標原点に最も近い帰着点の500 mm手前から停止を開始するよう変更した。実機ログで停止位置と正常減速を確認する。
- `lineAlpha_x1000`を小さな値から有効化して、自己位置ドリフトを抑制する比較走行を行う。
- 変更は1回の走行で最大2パラメータに限定し、10本以上のログで再現性を確認する。

## 今後の予防策

PATH REPLAY解析後は、`routeControllerVersion`、`routeSourceLog`、最終経路index、最終距離、`emcStop`を確認し、バージョン6では座標原点への帰着点の500 mm手前で停止シーケンスへ移ったことを確認する。実機検証完了までは`open`を維持する。
