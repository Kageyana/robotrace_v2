...
- 2026-09-13: 一次走行12407で発生した無効更新の原因候補だった共分散更新をJoseph形式へ置換し、更新前共分散を全要素退避してから新共分散を別行列で計算するよう修正した。融合出力は整数変換前にfloat値を±535 pulseで検証し、異常時は制限した生エンコーダへ戻す。`distanceKalman.outputGuardCount`を走行単位で保持して終了ヘッダへ追加した。共分散パラメータは変更していない。
