...
- 2026-09-26: 現行mainのオートスタート設定・ログ保存・停止処理を基準に、`codex/imu-distance-kalman` からPATH REPLAY Level 0とSHORTCUT Level 1の経路追従機能を選択移植した。距離カルマン融合は含めない。弧長・進行距離・方位で対応点を制限し、小R手前の旋回補正、実座標ラインセンサーの小幅姿勢補正、Level 1直線回廊、原点方向への半距離延長と延長点ゴールを追加した。新形式かつ正常終了・閉路有効・行数一致・IMU校正成功・距離換算一致の一次ログだけを経路元として許可し、自動走行も検証失敗時に停止する。`pathReplay` と `thetaGain_x1000` を設定末尾に追加し、旧形式は既存の有効値を保持して修復する。PCテストおよびDebug/Releaseビルドで確認した。実機未検証のため性能採否は未確定。
