...
/////////////////////////////////////////////////////////////////////
// モジュール名 pathCsvRowComplete
// 処理概要     CSV行が改行まで読み込まれた完全な行か確認する
// 引数         line: CSV行
// 戻り値       true:完全 false:切り詰めまたは未終端
/////////////////////////////////////////////////////////////////////
static bool pathCsvRowComplete(const char *line)
{
...
static bool pathReadColumnHeader(FIL *file, RouteCsvColumns *columns)
{
	...
	const bool supportedVersion = strstr(routeCsvLine, "pathSourceFormatVersion=1,") != NULL;
	...
	columns->closureValid = pathPolicySourceMetadataValid(
		supportedVersion, valid != NULL, (reason != NULL) ? 0U : 1U,
		mode != NULL, stop != NULL, (scaleEnd != NULL && *scaleEnd == ',') ?
		(uint32_t)sourcePulsePerMeter : 0U, calibrated != NULL,
		(calibrationSamples != NULL) ? IMU_CALIBRATION_SAMPLE_COUNT : 0U,
...
static bool pathExtendDriveRouteTowardOrigin(uint8_t shortcutLevel)
{
	...
	float goalExtensionMm = 0.0F;
	float goalX = 0.0F;
	float goalY = 0.0F;
	uint16_t appendCount = 0U;
	uint16_t availablePoints = (routeCount <= PATH_ROUTE_MAX_POINTS) ?
		(uint16_t)(PATH_ROUTE_MAX_POINTS - routeCount) : 0U;
	if (!pathPolicyComputeGoalExtension(endX, endY, PATH_ROUTE_SPACING_MM,
		availablePoints, &goalX, &goalY, &goalExtensionMm, &appendCount)) return false;
	float unitX = (goalX - endX) / goalExtensionMm;
	float unitY = (goalY - endY) / goalExtensionMm;
...
int16_t routeBuildFromLog(int logNumber, uint8_t shortcutLevel)
{
	...
	while (f_gets(routeCsvLine, sizeof(routeCsvLine), &file) != NULL)
	{
		...
		secondPassRows++;
		if (pulse < secondPassPreviousPulse)
		{
			routeOverflow = true;
			break;
		}
	...
	float maximumArcMm, PathLineMatch *match)
{
	...
	for (uint16_t i = begin; i < lastPoint; i++)
	{
		...
		if (!pathPolicyAssociationAllowed((float)driveRouteArcMm[i], maximumArcMm,
			segmentHeadingError, PATH_ASSOCIATION_HEADING_MAX_DEG)) continue;
...
void pathFollowerUpdateTarget5ms(void)
{
	...
	for (uint16_t i = begin; i <= end; i++)
	{
		...
		if (!pathPolicyAssociationAllowed((float)driveRouteArcMm[i], maximumArcMm,
			candidateHeadingError, PATH_ASSOCIATION_HEADING_MAX_DEG)) continue;
	...
	if (currentLineValid && associationValid)
	{
			...
			!corridorLineCorrectionBlocked)
		{
			...
			if (pathPolicyPoseCorrectionAllowed(correctionDistance, headingCorrectionDeg,
				PATH_LINE_POSITION_CORRECTION_MAX_MM, PATH_LINE_HEADING_CORRECTION_MAX_DEG))
			{
...
{
	if (routeIndex >= routeCount) return false;
	return pathPolicyGoalReached(pathGoalValid, routeIndex, routeCount,
		driveRouteArcMm[routeIndex], pathGoalArcMm);
}
/////////////////////////////////////////////////////////////////////
...
void readShortcutSettings(void)
{
	...
	int values[7] = {PATH_DEFAULT_MAX_LEVEL, PATH_DEFAULT_LOOKAHEAD_BASE_MM,
		...
		PATH_DEFAULT_KHEADING_X100, PATH_DEFAULT_LINE_ALPHA_X1000,
		PATH_DEFAULT_LINE_THETA_GAIN_X1E9};
	const int defaults[7] = {PATH_DEFAULT_MAX_LEVEL, PATH_DEFAULT_LOOKAHEAD_BASE_MM,
		PATH_DEFAULT_LOOKAHEAD_PER_MPS_MM, PATH_DEFAULT_KLATERAL_X100,
		PATH_DEFAULT_KHEADING_X100, PATH_DEFAULT_LINE_ALPHA_X1000,
		PATH_DEFAULT_LINE_THETA_GAIN_X1E9};
	const int minimums[7] = {0, PATH_LOOKAHEAD_BASE_MIN_MM, 0, 0, 0, 0, 0};
	const int maximums[7] = {PATH_LEVEL_MAX, PATH_LOOKAHEAD_BASE_MAX_MM,
		PATH_LOOKAHEAD_SPEED_MAX_MM, PATH_KLATERAL_MAX_X100,
		PATH_KHEADING_MAX_X100, PATH_LINE_ALPHA_MAX_X1000,
		PATH_LINE_THETA_GAIN_MAX_X1E9};
	shortcutSettings = (ShortcutSettings){PATH_DEFAULT_MAX_LEVEL,
		PATH_DEFAULT_LOOKAHEAD_BASE_MM, PATH_DEFAULT_LOOKAHEAD_PER_MPS_MM,
		PATH_DEFAULT_KLATERAL_X100, PATH_DEFAULT_KHEADING_X100,
		PATH_DEFAULT_LINE_ALPHA_X1000, PATH_DEFAULT_LINE_THETA_GAIN_X1E9};
	bool repair = false;
	int parsed = 0;
	...
	bool present[7];
	for (uint8_t i = 0U; i < 7U; i++) present[i] = (int)i < parsed;
	repair = pathPolicyMergeSparseSettings(values, present, defaults,
		minimums, maximums, 7U) || repair;
	shortcutSettings.maxLevel = (uint8_t)values[0];
	shortcutSettings.lookaheadBaseMm = (uint16_t)values[1];
	shortcutSettings.lookaheadPerMpsMm = (uint16_t)values[2];
	shortcutSettings.kLateral_x100 = (uint16_t)values[3];
	shortcutSettings.kHeading_x100 = (uint16_t)values[4];
	shortcutSettings.lineAlpha_x1000 = (uint16_t)values[5];
	shortcutSettings.lineThetaGain_x1e9 = (uint16_t)values[6];
