...
- IMU温度係数の恒温槽計測は、1ms集計・2秒ごとの `elapsed_ms,temp_C,gyroZMean_dps,gyroZStd_dps` 保存を行う。SD書き込みはメインループで実施する。計測時は拡張UI、モーター、左右エンコーダ、マーカーセンサ、ラインセンサ、LiPoを外し、mainボード、BMI088、microSD、mainボード左右ボタン、フルカラーLED、外部電源だけを接続する。拡張UIの検出結果に依存せず、待機中に左ボタンを1秒長押しすると電源再投入まで専用モードを保持して記録を開始し、右ボタンを1秒長押しすると記録を終了する。停止後は左ボタンの再長押しで次の連番ファイルを開始する。専用モードでは通常走行ボタンを無視し、モーター・吸引ファンPWM、エンコーダタイマ、ライン／マーカーセンサ電源・TIM3・ADC取得、通常制御を停止する。開始成功は緑、SD同期・クローズ完了は青、異常は赤で2回点滅し、それ以外は消灯する。解析は `.agents/skills/robotrace-log-analysis/SKILL.md` の恒温槽計測条件に従う。
...
- 2026-09-12: 拡張UI、モーター、左右エンコーダ、マーカーセンサ、ラインセンサを外す恒温槽計測用に、mainボード左ボタンの1秒長押しで開始し、右ボタンの1秒長押しで終了する、電源再投入まで保持される専用モードを追加した。専用モードでは不要なPWM、タイマ、ADC、通常制御を停止し、IMU温度ログは4列CSVへ変更した。開始時緑、終了時青、異常時赤のフルカラーLED点滅を行う。
