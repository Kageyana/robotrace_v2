"""Compute voltage-command and control metrics for logs 11407-11421."""
...
OUT_PATH = Path(r"D:\robotrace\robotrace_v2\analysis\log_11407_11421_voltage_command_metrics.csv")
PULSE_METER = 53424.0
...
def metric_row(number):
    ...
    target_values = [parse_float(row.get("targetSpeed")) for row in rows]
    speed_values = [parse_float(row.get("encCurrentN")) for row in rows]
    speed_errors = [
        (target - speed) / PULSE_METER * 1000.0
        for target, speed in zip(target_values, speed_values)
    ...
    command_values = [
        value
        for row in rows
        for value in (parse_float(row.get("motorVoltageCmdL_mV")), parse_float(row.get("motorVoltageCmdR_mV")))
...
def main():
    rows = [metric_row(number) for number in range(11407, 11422)]
