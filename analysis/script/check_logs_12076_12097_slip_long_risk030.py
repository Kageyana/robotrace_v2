"""Validate CA_SLIP_DOWN_LONG_RISK=0.03 logs by autoStart run number."""
...
def main() -> None:
    ...
    for log_number, log in logs.items():
        ...
        valid, reason = base.valid_log(log, 2 if is_secondary else 0, 150.0)
        summary = {
            "log": log_number,
    ...
    for auto_start in (2, 3, 4, 5):
        old = [
            ...
            if row["condition"] == "speed_ff150_baseline"
            and int(float(row["autoStart"])) == auto_start
        ]
        new = [row for row in run_rows if int(float(row["autoStart"])) == auto_start]
        result = {"autoStart": auto_start, "baseline_n": len(old), "candidate_n": len(new)}
        for field in (
            "last_cnt_ms",
            "speed_error_target_minus_actual_mean_mps",
            ...
            result[f"candidate_{field}"] = new_value
            result[f"candidate_minus_baseline_{field}"] = new_value - old_value
        comparison.append(result)

    set_rows = []
    for set_id, group in enumerate(complete, 1):
        secondary = [
            summaries[log_number]
            for log_number in group
            if int(summaries[log_number]["autoStart"]) != 1
        ]
        set_rows.append({
    ...
    excluded_rows = [{"condition": LABEL, **row} for row in excluded]
    prefix = "log_12076_12097_slip_long_risk030"
    base.write_csv(base.OUT_DIR / f"{prefix}_summary.csv", list(summaries.values()))
    write_csv(base.OUT_DIR / f"{prefix}_by_autostart.csv", run_rows)
    write_csv(base.OUT_DIR / f"{prefix}_comparison.csv", comparison)
    write_csv(base.OUT_DIR / f"{prefix}_sets.csv", set_rows)
    write_csv(base.OUT_DIR / f"{prefix}_excluded_sets.csv", excluded_rows)
    ...
    for row in comparison:
        print(
            f"autoStart={row['autoStart']} n={row['baseline_n']}/{row['candidate_n']} "
