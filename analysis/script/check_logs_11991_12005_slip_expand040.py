"""Validate CA_SLIP_EXPAND_1=0.40 logs and compare by autoStart run number."""
...
NEW_LOGS = list(range(11991, 12006))
...
def main() -> None:
    ...
    for set_id, group in enumerate(complete, 1):
        for log_number in group:
            if int(summaries[log_number]["autoStart"]) == 1:
                continue
            row = {"condition": "slip_expand_040", "set": set_id}
    ...
    excluded_rows = [{"condition": "slip_expand_040", **row} for row in excluded]

    base.write_csv(base.OUT_DIR / "log_11991_12005_slip_expand040_summary.csv", list(summaries.values()))
    write_csv(base.OUT_DIR / "log_11991_12005_slip_expand040_by_autostart.csv", run_rows)
    write_csv(base.OUT_DIR / "log_11991_12005_slip_expand040_comparison.csv", comparison)
    write_csv(base.OUT_DIR / "log_11991_12005_slip_expand040_sets.csv", set_rows)
    write_csv(base.OUT_DIR / "log_11991_12005_slip_expand040_excluded_sets.csv", excluded_rows)
