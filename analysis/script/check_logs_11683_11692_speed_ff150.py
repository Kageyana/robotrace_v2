"""Check the newest speed FF=150 logs by complete autoStart set and run number."""
...
def load_rows(number):
    path = base.LOG_DIR / f"{number}.csv"
...
def summarize(number):
    ...
    if row["valid"] and row["rows"] < MIN_ROWS:
        row["valid"] = False
        row["reason"] = f"rows<{MIN_ROWS}"
    if row["valid"] and row["last_cnt_ms"] < MIN_LAST_CNT_MS:
        row["valid"] = False
        row["reason"] = f"last_cnt_ms<{MIN_LAST_CNT_MS}"

    if row["valid"] and row["optimalTrace"] != "0.00":
        _, data = load_rows(number)
        max_index = max(
            (int(float(item.get("optimalIndex", -1))) for item in data if item.get("optimalIndex", "")),
            default=-1,
        )
        row["optimalIndex_max"] = max_index
        if max_index < MIN_OPTIMAL_INDEX:
            row["valid"] = False
            row["reason"] = f"optimalIndex_max<{MIN_OPTIMAL_INDEX}"
    else:
        row["optimalIndex_max"] = ""
    row["condition"] = "kp12_ff150"
    return row
            ...
            output.append(
                {
                    ...
                    "rows": row["rows"],
                    "optimalIndex_max": row["optimalIndex_max"],
...
def main():
    ...
    summary_path = OUT_DIR / "log_11683_11692_speed_ff150_summary.csv"
    runs_path = OUT_DIR / "log_11683_11692_speed_ff150_by_autostart.csv"
    excluded_path = OUT_DIR / "log_11683_11692_speed_ff150_excluded_sets.csv"
