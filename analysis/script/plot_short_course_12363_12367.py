...
for n in range(12363,12368):
 ...
 result.append(dict(log=n,rows=len(rows),malformed_rows=sum(len(r)<len(cols) for r in raw),time_s=t[-1],distance_mm=rows[-1]['encTotalOptimal']/PULSE_MILLIMETER,dt_min=min(dt),dt_max=max(dt),speed_error_p95_mps=percentile([abs(r['encCurrentN']-r['targetSpeed'])/PULSE_MILLIMETER for r in rows],.95),slip_long_samples=sum(r['slipFlag']!=0 for r in rows),slip_lat_samples=sum(r['slipFlagLat']!=0 for r in rows),index_last=rows[-1]['optimalIndex'],battery_min_V=min(r['batteryVoltage_mV'] for r in rows)/1000))
 axxy.plot([r['x'] for r in rows],[r['y'] for r in rows],label=str(n))
 if n==12363: continue
 for ax,key,scale in zip(axs,['encCurrentN','targetAngularvelo','pathErrorY_mm','pathErrorHeading_cdeg','slipFlagLat'],[1/PULSE_MILLIMETER,1,1,.01,1]):
...
axs[0].axhline(54/PULSE_MILLIMETER,color='black',ls='--',label='target')
